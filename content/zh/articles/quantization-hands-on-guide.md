...
### Q1: AWQ、GPTQ 与 GGUF 在工程落地中应该如何选型？
如果在 Linux 服务器上使用高性能推理引擎，优先选择 **AWQ**（激活感知保护核心权重，困惑度最低）或 **GPTQ**；如果在个人电脑或边缘端部署，优先选择 **GGUF** 并搭配 llama.cpp 或 Ollama；而在现代数据中心云端集群，原生 **FP8** 则是综合吞吐与精度的首选。详细分布式集群部署方法可参考 [vLLM 生产级部署全指南](/articles/vllm-serving-guide/)。

### Q2: 为什么 4-bit 量化后模型显存占用降低了，推理速度却没有提升？
权重量化（W4A16）在计算时需要在 GPU 显存或 SRAM 中将 4-bit 权重临时反量化（Dequantize）回 FP16/BF16，因此计算延迟（Compute-bound）并未减少。量化的提速收益主要体现在显存带宽受限（Memory-bound）的单 Batch 或低并发场景；若在高并发批处理场景下，推荐采用 W8A8 或 FP8 全量化方案以调用专用 Tensor Core。

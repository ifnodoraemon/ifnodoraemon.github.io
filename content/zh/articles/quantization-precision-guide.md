...
举个极端的例子：Llama 4 Maverick 400B 即使使用 FP16 精度，单单存下权重就需要约 800GB 显存——也就是说，就算你把一台价值百万的 8 卡 H100（8×80GB=640GB）塞满都装不下它（至少需要 8 卡 H200 或 16 卡 H100 集群），更别提推理了。哪怕是 70B 的模型，也需要约 140GB 显存，远超单张显卡的极限。
...
│   ├─ 已配最新 Blackwell B200 → NVFP4（≤1%精度损失，且吞吐是 FP8 的 2 倍）
...
- 如果你要把它挂在服务里，**想让出词速度爆表** → 找 **GPTQ** 模型配合 [vLLM 生产级部署](/articles/vllm-serving-guide/)
...
1. **自己玩**：毫不犹豫拥抱 AWQ / GGUF 的 4-bit，配合 [大模型量化实战指南](/articles/quantization-hands-on-guide/) 用极小的代价越级体验高配模型。
2. **老板让搭建并发推理服务（有新机器）**：抛弃一切复杂的 SmoothQuant，在 [vLLM 生产级部署全指南](/articles/vllm-serving-guide/) 里无脑跑 FP8，那是官方钦定的最优解。
...
### Q1: 在大模型训练与推理中，FP16 与 BF16 该如何抉择？
除非运行在不支持原生 bfloat16 算力的老旧 GPU 架构（如 Pascal 或部分 Volta 显卡）上，否则强烈优先选择 **BF16**。BF16 与 FP32 拥有完全一致的 8 位指数动态范围（Dynamic Range），在训练与微调中几乎不会产生梯度溢出（Underflow/Overflow），彻底免去了 FP16 繁琐的动态损失缩放（Loss Scaling）逻辑；在生产部署高吞吐推理时，直接参考 [vLLM 生产级部署全指南](/articles/vllm-serving-guide/) 使用 BF16 即可获得高稳定度与优异吞吐。

### Q2: 工业落地中，通常将多少精度损失作为模型量化上线的可接受阈值？
通用工程验收标准通常要求 WikiText-2 等基准困惑度（Perplexity, PPL）增量控制在 0.1~0.2 以内，且核心业务评测集（如 MMLU 知识问答、GSM8k 数学推理）准确率掉点不超过 1%。对于闲聊机器人或开放式文本总结场景，掉点 2%~3% 的 4-bit 量化方案仍具备极高实用价值；但如果是写代码或复杂数值计算等强逻辑场景，建议参考 [大模型量化实战指南](/articles/quantization-hands-on-guide/) 选用具备关键激活值保护的 AWQ 或原生 FP8 方案。

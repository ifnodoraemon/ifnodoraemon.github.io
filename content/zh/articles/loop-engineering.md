...
### Q1: Loop工程中的终止条件通常如何设计？
终止条件必须是机器可客观判定的指标。在Agent开发中，通常结合测试覆盖率或独立验证节点的返回结果来判定，避免主观偏差。更多实践请参考[做 AI Agent 的 7 条运行时实践](/articles/agent-runtime-practices/)。

### Q2: 为什么ReAct架构容易陷入死循环？
因为每一次失败尝试都会被追加到上下文中，导致上下文污染并放大模型的错误偏见。通过Loop工程的独立裁判(Checker)机制隔离上下文，并设置安全闸门，可以有效阻断死循环。

...
到了这一层，开发重点转移到了围绕模型搭建的**安全线束/脚手架 (Harness)** 上。例如利用 LangGraph 等框架设计严格的状态机边界（具体代码落地可参考 [从零搭建 AI Agent 应用](/articles/build-ai-agent/)）。
...
3. **执行约束 (HARNESS)**：
   沿用第三层的所有安全防护、工具调用和沙箱执行机制（系统化工程实践详见 [做 AI Agent 的 7 条运行时实践](/articles/agent-runtime-practices/)）。
4. **独立裁判 (CHECKER)**：
...
### Q1: 基于循环的 Loop 架构与基于 DAG（有向无环图）的工作流编排有什么根本区别？
DAG 工作流的核心假设是任务路径在执行前已知，节点间依赖关系完全静态确定，适合步骤固定的自动化流水线；而 Loop 架构针对的是探索性、高不确定性的长程任务，依赖“目标-驱动-约束-检验-闸门”构成的闭环自适应收敛。在工业级实践中，两者通常相互结合：外层由 DAG 或状态图管理全局骨架，节点内部由自主收敛的 Loop 驱动执行（可参考 [从零搭建 AI Agent 应用](/articles/build-ai-agent/)）。

### Q2: 如何从机制上彻底根除 Agent 在失败尝试中的死循环 (Doom-loop)？
死循环的根源在于失败历史污染了上下文，使得模型基于错误轨迹产生认知偏见，进而反复调用同一工具。解决的根本策略在于控制面外推：设置物理熔断安全闸门（硬性步数上限与 Token 预算）、引入上下文隔离的独立 Checker 评估产物，并在 Runtime 层实施重复调用与无进展检测（深度实践详见 [做 AI Agent 的 7 条运行时实践](/articles/agent-runtime-practices/)）。

### Q3: 什么时候应该采用多智能体 (Multi-Agent) 协同，什么时候坚持单智能体？
如果任务具有强序列依赖且各步骤共享上下文，单智能体配合独立的 Maker-Checker 验证链是稳定性和性价比最高的方案。只有当子任务具有绝对清晰的职责边界、完全独立的上下文空间，且需要隔离不同工具的权限时，才推荐引入多智能体委派架构；盲目拆分子 Agent 只会放大通信开销与状态漂移风险。

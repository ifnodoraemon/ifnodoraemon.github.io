...
跨工具通用是 `AGENTS.md` 的核心优势。无论你用 Cursor、Copilot、Claude Code 还是 Aider，它们都会自动读取项目根目录下的这个文件。一次配置，全平台生效。如果你需要针对特定工作流注入更专业的模块化技能或自动化脚本，可进一步参阅 [Skills 深度解析：给 AI 编程助手装上「专业大脑」](/articles/skills-guide/)。
...
LLM 的上下文窗口不是"越大越好"。即使 [Gemini 3.1 Pro 拥有高达 1M~2M token 上下文](/articles/ai-trends-2026/)，模型对长上下文中间位置信息的注意力仍然最低。更重要的是——每轮对话的历史消息都在消耗你的有效上下文空间。30 轮对话 × 每轮平均 2000 token ≈ 60K token 的历史噪音，留给真正重要信息的空间越来越少。
...
### Q1: 项目中的 `AGENTS.md` 规则文件与模块化 `Skills` 应该如何协同配合？
`AGENTS.md` 适合作为项目的全局架构规范与安全红线（严格控制在 200 行以内），定义统一的代码风格和禁忌操作；而具体的垂直复杂任务（如数据库 Schema 迁移、特定框架重构或 CI 流水线生成）则更适合封装为可插拔的模块化技能。深入掌握技能定义机制与主流平台集成，可参考 [Skills 深度解析：给 AI 编程助手装上「专业大脑」](/articles/skills-guide/)。

### Q2: 为什么我的 AI 编程工具总是无法精准读取数据库或调用外部工具？
这通常是因为工具与开发环境之间缺乏标准化的上下文通信协议。2026 年主流的工业解法是通过 MCP（Model Context Protocol）将本地数据库、Git 仓库和 API 封装为标准服务，让 Cursor、Claude Code 等助手按需安全调用。关于协议规范与服务搭建，请参阅 [MCP 协议完全指南：从原理到实战](/articles/mcp-guide/)。

### Q3: 在大型历史遗留代码库中，AI 总是引入非预期的破坏性重构，如何破局？
首要原则是“先写测试再写实现（TDD）”并强制执行“只读规划（Plan Mode）”。在第一轮提示中明确禁止 AI 生成任何实现代码，仅要求其产出包含修改文件清单与依赖分析的 `PLAN.md`；待人工审查确认边界后，再启动独立会话分块执行代码修改并运行测试，避免大范围无约束扩散。

...
### Q1: Few-shot 和 Zero-shot 的工程权衡是什么？
在实际应用中，Zero-shot 适合基础指令明确、无需特殊格式的任务，能够节省上下文 Token。而 Few-shot 在需要特定输出格式（如 JSON）或特定业务语气的场景下表现更佳。在上下文窗口有限时，可以结合 [RAG 实战](/articles/rag-in-practice/) 动态检索示例，这被称为动态 Few-shot。

### Q2: System Prompt 的最佳实践有哪些？
System Prompt 应该设定明确的角色、职责边界和约束条件。不要在系统提示词中硬编码过多的背景知识，而是应该使用系统化的 [上下文工程](/articles/context-engineering-guide/) 来按需提供背景信息。同时，使用 Markdown 结构化你的 System Prompt 可以帮助模型更好地理解指令层级。

...
*   **幂等性（Idempotency）设计**：在重试机制下，一个 Agent 可能会被唤醒多次去执行同一个工具（比如“扣减积分”）。为了防止脏数据，工具调用层必须实现严格的 Exactly-Once 语义，基于特定的 `Task_ID + Step_ID` 颁发请求令牌（Token）。关于工具边界隔离与运行时状态设计，可参考 [做 AI Agent 的 7 条运行时实践](/articles/agent-runtime-practices/)。
...
在 2026 年，无 Evals（自动评估指标）不谈生产。关于如何破除公开榜单刷分并搭建业务强绑定的评估体系，可深度参考 [拒绝榜单刷分：如何构筑契合业务的 LLM 评估体系](/articles/llm-evaluation-guide/)。
...
### Q1: 当多 Agent 协同任务链路很长时，如何从架构上防止“步步衰减”导致的整体崩塌？
核心在于切断单向长依赖链，引入分阶段状态快照（Checkpointing）与分治验证子智能体（Verifier Agent）。每个阶段完成后必须执行确定性校验（如输出类型检查、Schema 校验或单元测试），未通过则在当前节点局部自愈或回滚，而不是任由脏数据向下游扩散。运行时状态管理与工具隔离细节可参考 [做 AI Agent 的 7 条运行时实践](/articles/agent-runtime-practices/)。

### Q2: 使用 LLM-as-a-Judge 进行回归测试时，如何以最低成本获得高统计置信度？
优先使用校准后的多轮盲测打分（如交换输入顺序以对消位置偏见），并结合 G-Eval 的 Token Logprob 加权获得平滑连续的浮点评分；对于核心业务场景，定期抽取 5%~10% 的自动评分结果交由领域专家进行混淆矩阵比对与对齐校准。深入了解裁判模型的数学偏差与实操技巧，请参考 [大模型业务评估体系建设指南](/articles/llm-evaluation-guide/)。

### Q3: 生产级分布式 Agent 编排为什么普遍选用 Temporal 等外部工作流引擎而非纯内存状态机？
纯内存状态机在面对网络波动、Pod 重启或大模型 API 速率限制时缺乏持久化韧性，极易导致正在进行的长流程任务状态丢失或重复扣费。基于事件驱动的外部工作流引擎提供持久化定时器、确定性重试队列与状态序列化机制，能够天然保障分布式任务的幂等性与 Exactly-Once 执行。
...
### Q4: 如何解决模型评估中成本过高的问题？
建议在关键路径上使用更强（且更贵）的模型进行评估，而在普通的次要路径上，可以使用蒸馏后的小模型结合特定的 [LLM 评估指标](/articles/llm-evaluation-guide/) 进行批量自动化测试。

### Q5: 为什么不能完全依赖公共榜单？
公共榜单的数据极易被模型在预训练或微调阶段“刷榜”污染。构建自己私有业务场景下的 Evals 体系才能真实反映系统的可用性，具体可参考 [大模型业务评估指南](/articles/llm-evaluation-guide/)。

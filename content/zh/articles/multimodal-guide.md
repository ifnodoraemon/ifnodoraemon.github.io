...
### Q1: 多模态大模型如何处理长视频输入？
目前如 Gemini 3.1 Pro 等模型可以原生支持长视频理解。在实际应用中，开发者可以结合[大模型提示工程实践](/articles/prompt-engineering-guide/)，提取关键帧或分段上传视频，以提高大模型的分析准确度和效率。

### Q2: 视觉 Token 的计算方式是什么？
多模态模型通常基于 Vision Transformer (ViT) 将图片切割为 Patch。高分辨率图像会导致 Token 消耗成倍增加，因此建议在后端执行降采样，以平衡成本和解析精度。

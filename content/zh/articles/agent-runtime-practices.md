...
换句话说，后端更像 runtime，而不是 workflow engine。它负责告诉模型“你现在能看到什么、能做什么、哪些结果不能落地”，但不负责把“下一步应该做什么”偷偷编码进去。关于如何通过状态机与 LangGraph 构建具备自主决策的 Agent，可参考 [从零搭建 AI Agent 应用](/articles/build-ai-agent/)。
...
这其实是在把 Agent 重新做成阶段机。关于打破这种流程化限制，可以进一步了解[循环工程架构](/articles/loop-engineering/)。
...
这类优化没有换模型那么显眼，但它们对 agent 稳定性的影响通常更大。很多“推理问题”，最后本质上都是上下文工程问题（系统化的治理技巧可参考 [上下文工程实战指南](/articles/context-engineering-guide/)）。
...
### Q1: 生产环境中如何彻底防范 Agent 陷入死循环 (Doom-loop)？
防范死循环需要多层防御：在 Runtime 层设置严格的步数上限（Max Steps）与 Token 预算门槛；引入“无进展检测”（如连续 3 次调用同一工具或输出高度相似的参数时触发中断）；并在架构上实施独立于主上下文的裁判（Checker）机制。关于收敛控制系统的完整设计，请参考 [从 Prompt 到 Loop 工程架构演进](/articles/loop-engineering/)。

### Q2: 工具调用发生超时或第三方 API 熔断时，Runtime 应如何优雅处理？
Runtime 必须为所有工具执行配置严格的异步超时（如 10-30 秒）与熔断机制，避免阻塞 Agent 主线程。当工具超时或报错时，不要将底层堆栈原样抛出，而应将其包装为包含具体失败原因的结构化 Observation 错误信息（例如“查询超时，建议缩小时间范围重试”），让模型具备自主修正或降级切换工具的机会。

### Q3: 面对长程任务中的上下文窗口溢出，最佳的治理策略是什么？
严禁采用简单的全量历史滑动窗口截断，这会导致关键系统指令或早期事实丢失。推荐采用分级治理：工具调用结果及时摘要化、利用 Working Memory 显式沉淀已验证结论、对早期非核心轮次进行周期性压缩归纳（Digest）。更系统化的上下文治理方案可参阅 [上下文工程全指南](/articles/context-engineering-guide/)。

---

## 常见问题 (FAQ)

### Q4: Agent Runtime 如何处理异常流？
在处理异常时，Runtime 应该捕获错误并作为系统状态或观察结果返回给模型，而不是直接崩溃。这种设计允许模型根据上下文自主决定下一步行动，也可以参考[循环工程](/articles/loop-engineering/)进行异常处理优化。

### Q5: 为什么不建议将业务逻辑硬编码在 Runtime 中？
硬编码会削弱大模型的推理和决策能力，将其降级为流程执行器。更优雅的做法是将这些逻辑设计为带约束的工具，让 Agent 在安全边界内自由探索。
